...
import {
    ...
    URI as URIEvent,
} from '../generated/templates/DappToolsRegisteredERC1155/IERC1155';

import { constants, decimals, events, transactions } from '@amxx/graphprotocol-utils';

import { fetchAccount } from './fetch/account';

import { fetchERC1155, fetchERC1155Token, fetchERC1155Balance, fetchERC721Operator, replaceURI } from './fetch/erc1155';
...
export function handleTransferSingle(event: TransferSingleEvent): void {
    ...
    registerTransfer(event, '', contract, operator, from, to, event.params.id, event.params.value);
}
...
export function handleTransferBatch(event: TransferBatchEvent): void {
    ...
    if (ids.length == values.length) {
        for (let i = 0; i < ids.length; ++i) {
            registerTransfer(event, '/'.concat(i.toString()), contract, operator, from, to, ids[i], values[i]);
        }
    }
}
