// https://github.com/unstoppabledomains/uauth/blob/main/packages/web3modal/src/index.ts

import type UAuthSPA from '@uauth/js';
import type { UAuthConstructorOptions, UserInfo } from '@uauth/js';
import Web3Modal, { connectors, IAbstractConnectorOptions, IProviderDisplay } from 'web3modal';

export interface IUAuthOptions extends Partial<IAbstractConnectorOptions>, UAuthConstructorOptions {
...
export const getUAuth = (UAuth: typeof UAuthSPA, opts: IUAuthOptions): UAuthSPA => {
    return new UAuth(opts);
};

export const connector = async (UAuth: typeof UAuthSPA, opts: IUAuthOptions): Promise<any> => {
    ...
    if (['web3', 'injected'].includes(user.wallet_type_hint)) {
        ...
        provider = connectors.walletconnect((w3m as any).providerController.getProviderOption(id, 'package'), {
